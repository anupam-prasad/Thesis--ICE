...
possible neural subtractions that take place in the animal's brain in order to enhance directional sensitivity. We will end this section
...
\begin{equation}\label{newsoundinput}
 p_0=pe^{j\omega t} e^{j\Delta/2},\qquad p_L=pe^{j\omega t} e^{-j\Delta/2},\qquad \Delta=1.5kL\sin\theta.
\end{equation}
\subsection{Membrane Vibration Velocity}
In this section we will see how the total membrane displacements accurately
represent the response of the membrane to external stimuli. The main advantage of these quantities is obvious - they only depend on direction and frequency. In Fig. \ref{hemidactylusvibampfull},
...
As we can see, the model qualitatively reproduces the behaviour fairly well. The directional dependence is obvious - the membrane has a distinctly higher
...
\end{figure}
As we can see the ipsilateral response is generally higher than the contralateral response and the difference peaks at a certain frequency.
...
\centering
  ...
  The amplitude response to ipsilateral stimuli is generally higher and is at a maximum when the object is nearest to the ear i.e. $\theta=90^\circ$. The dotted line denotes a sound source directly in front or behind the animal and the dashed lines denote contralateral stimuli.}
  \label{freqdepboth}
\end{figure}
...
\begin{figure}[ht]
\centering
  \includegraphics[width=.6\linewidth]{Diagrams/Plots/directionplots/directionplots.png}
  \caption[Direction dependence of membrane vibration amplitudes for different frequencies.]{Direction dependence of membrane vibration amplitudes  in the ICE Model for different frequencies - common house gecko (left), Tokay gecko (right).
  The variation of the amplitude as the sound source completes a full circle around the animal is illustrated.}
...
is also consistent with observations (\cite{dalsgaardmanley1}, \cite{dalsgaardmanley2}).

The significance of our analysis upto this point should
already be apparent. Due to the relatively small head sizes of the geckos, the sound arriving at the two ears differ very slightly in phase and not at all in amplitude. On the other hand
 these animals overcome the problem and create direction dependent differences between the vibration amplitudes of the membranes through the use of coupled ears.
...
we need functions that quantify the directional dependence of the system. As a first step in this direction, we 
calculate the displacement ratio of the membranes,
\begin{align}
...
We can now define the two quantities that are the main focus of this chapter - the internal level difference (iLD) and internal time difference (iTD) -
in terms of the displacement ratio. 
\begin{align}
  iLD&\defeq 20\mbox{Log}_{10}\left(\left|\frac{\dot{S}^0}{\dot{S}^L}\right|\right)\\
  iTD&\defeq \mbox{Arg}\left(\frac{\dot{S}^0}{\dot{S}^L}\right)/\omega.
\end{align}
Due to the steady state approximation we have the added advantage that the ratio of the displacement amplitudes is equal
to that of the velocity amplitudes although the velocity amplitudes are easier to measure experimentally. The iLD measures the ratio between the amplitudes of the eardrums and is the same as the IVAD function defined by J\o{}rgensen et al. \cite{jorgensenschmitz} and
...
with respect to the contralateral ear.
...
between the vibrations of the eardrums without coupling and diffraction effects. Due to the 
...
a priori Inter. In larger animals
like humans the difference between the input amplitudes increases with frequency due to 
diffraction effects (shadowing); cf. \cite[p~.154]{fletcheracoustic}.
...
need to account for the motion of the extracolumella. Moreover, the full mechanics of the extracolumella including 
 its flection at higher frequencies can have significant effects.
% \begin{figure}[ht!]
%  \centering
