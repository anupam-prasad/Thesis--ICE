...
\end{figure}
The solution for forced membrane vibrations follows in the same way as in the circular membrane case.
...
v_x=-\begin{cases}
	      \frac{1}{\rho\omega}\displaystyle\sum^\infty_{q=0}\displaystyle\sum^\infty_{s=0}\zeta_{qs}\left(A_{qs}e^{j\zeta_{qs}x}-B_{qs}e^{-j\zeta_{qs}x}\right)f_{qs}(r,\phi)e^{j\omega t} &\mbox{, } r<a_{tymp}\\
	      0 &\mbox{, } a_{tymp}\leq r\leq a_{cyl}
           \end{cases}
\end{equation}
