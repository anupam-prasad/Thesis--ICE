...
For low frequencies (below $4$kHz), the extracolumella 
...
\begin{figure}[ht]
 ...
 $\Delta$. In this case, however, we have accounted for the influence of the head shape on the phase difference between the inputs
 to both ears.}
 \label{newheadmodel}
\end{figure}
...
vibration profiles, the ipsi- and contraleteral filters and the pressure coefficients. We will thus find an expression for the pressure 
...
equation - that describes the pressure distribution inside the cylindrical cavity. The pressure will be subject
to a ``no-penetration'' boundary condition - this is a physical requirement that results from the fact that the air
...
models to the loaded tympanum. The final expression will correspond to the steady-state vibrations of a linear elastic
membrane. As we neglect the transient response of the membrane, we will also discuss the circumstances under which this
is justified.

The main equations
of our model are a pair of coupled, inhomogenous $2^{nd}$-order Partial Differential Equations that describe the motion of the two
tympanic membranes driven by internal and external pressures. 
...
The approximate boundary conditions are given by,
\begin{align}
   \frac{\rho\omega^2}{\pi a^2_{cyl}}S_{0}&=\displaystyle\sum^\infty_{q=0}\displaystyle\sum^\infty_{s=0}j\zeta_{qs}\left(A_{qs}-B_{qs}\right)f_{qs}(r,\phi) \label{bcfinal1}\\
   \frac{\rho\omega^2}{\pi a^2_{cyl}}S_{L}&=-\displaystyle\sum^\infty_{q=0}\displaystyle\sum^\infty_{s=0}j\zeta_{qs}\left(A_{qs}e^{j\zeta_{qs}L}-B_{qs}e^{-j\zeta_{qs}L}\right)f_{qs}(r,\phi) \label{bcfinal2}
\end{align}
where $S_{0/L}$ is the total displacement of the membranes given by,
...
and $B_{qs}$,
\begin{align}
 &A_{qs}-B_{qs} = K_{qs}\frac{\rho\omega^2}{\pi a^2_{cyl}}S_{0}\\
 &A_{qs}e^{j\zeta_{qs}L}-B_{qs}e^{-j\zeta_{qs}L} = - K_{qs}\frac{\rho\omega^2}{\pi a^2_{cyl}}S_{L}\\
...
the more obvious fact of the cosine function integrating to zero. As a result we have $A_{qs}=B_{qs}=0$ for all
modes except the $(0,0)$ or the plane-wave mode. We will subsequently omit the subscripts ``$00$'' for these coefficients.
...
\begin{equation}\label{finalpressurecoefficients}
 A=\frac{\rho\omega^2K_{qs}}{2\pi a^2_{cyl}\sin kL}\left(S_0e^{-jkL}+S_L\right),\qquad  B=\frac{\rho\omega^2K_{qs}}{2\pi a^2_{cyl}\sin kL}\left(S_0e^{jkL}+S_L\right)
\end{equation}
we have also directly substituted the value of $\zeta_{00}=k$ in the above expressions.
