...
the eardrums and the amplification of hearing cues. This will be discussed in-depth in Chapter \ref{modelanalysischapter}.
...
the three-dimensional system but in our analysis this is unnecessary. The main reason for this is 
...
\centering
\captionof{table}{Parameters and Functions used in the ICE Model} \label{parametertable} 
...
$p_{0/L}$ & Sound pressure input on the ipsi- and contralateral ears respectively.\\
& \\
...
$u_{0/L}(r,\phi;t)$ & Membrane displacement function. The subscripts denote the ipsilateral ($x=0$) and contralateral ($x=L$) membranes.\\
...
by defining the ipsi- and contralateral filters. We will thus have the final expressions that give us the vibrations 
of the tympanic membranes as a function of the input pressure. We have listed the main parameters and functions used in our analysis (including the geometry parameters) in Table \ref{parametertable}.
...
We first exppand $U_{0}$ and $U_{L}$ in the orthogonal basis of the functions $f_{qs}$,
\begin{align}
 U_{0/L}(r,\phi;t)&=\displaystyle\sum^\infty_{q=0}\displaystyle\sum^\infty_{s=0} S^{0/L}_{qs}f_{qs}(r,\phi)e^{j\omega t}\\
 \mbox{where, }S^{0/L}_{qs}&=\frac{\int dS U^{0/L}f_{qs}(r,\phi)}{\int dS f^2_{qs}(r,\phi)}\nonumber
\end{align}
...
\end{align}
The next step will be to multiply both sides of \eqref{pluscoeffdef} and \eqref{minuscoeffdef} with the integral $\int dS u_{mn}$, and
...
,\qquad\frac{1}{\Lambda}=\displaystyle\sum^\infty_{m=0}\displaystyle\sum^\infty_{n=1}\frac{\left(\int dS u_{mn}\right)^2}{\Omega_{mn}\int dS u^2_{mn}}\nonumber
\end{align}
...
is blocked. Similarly, the contralateral filter gives us its response when there is no ipsilateral stimulus.
