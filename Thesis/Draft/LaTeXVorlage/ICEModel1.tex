...
cavity but require it to have a volume which is equal to that of the realistic cavities in typical specimens. This volume is denoted by $V_{cav}$. 
...
$C^{0/L}_{mn}$ & Membrane vibration expansion coefficients.\\
...
\begin{align}
 C^{+}_{mn}&=\left[p_{+}+\frac{\rho\omega^2}{k}S^+\cot \frac{kL}{2}\right]\frac{K_{mn}}{\Omega_{mn}}\label{pluscoeffdef}\\
 C^{-}_{mn}&=\left[p_{-}-\frac{\rho\omega^2}{k}S^-\tan \frac{kL}{2}\right]\frac{K_{mn}}{\Omega_{mn}}\label{minuscoeffdef}\\
 \mbox{where, }K_{mn}&=\frac{\int dS u_{mn}}{\int dS u^2_{mn}}\label{Kmndef}
\end{align}
...
&\frac{1}{\Lambda}=\displaystyle\sum^\infty_{m=0}\displaystyle\sum^\infty_{n=1}\frac{K_{mn}}{\pi a^2_{cyl}\Omega_{mn}}\label{lamfirstdef}
\end{align}
...
in numerical analyses it has to be approximated by choosing an appropriate membrane mode cutoff. The choice of this
...
(or equivalently $\mu_{mn}$). As a result, we can express the summation over a single index. In general, for the frequency ranges of the animals
we are concerned with, we will not need to calcualate the summation beyond the the first $30$ eigenmodes.
...
\begin{align}
 ...
 &R_V=0,\qquad L_V=0,\qquad C_V=\frac{V_{cav}}{\rho c^2}
\end{align}
...
\begin{itemize}
 \item In terms of the membrane motion - we are able to account for the effect of asymetrically loaded extracolumella rendering it possible to describe the membrane motion is spatial detail.
