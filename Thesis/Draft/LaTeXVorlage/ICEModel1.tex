...
and was based on methods presented in \cite{fletcheracoustic}. The problem was treated using the analogy of electrical circuits
and dealt with low-frequency and high-frequency regimes separately.
...
In these models, the sound inputs are treated as voltage sources and the system is broken down into components, e.g.	 membranes,
 cavities, apertures which are modelled as impedances. Their values depend on the geometry and material properties and in general
 can be resistive and reactive. The ``current'' is given by something called the acoustic flow which has the dimensions of
 volume per unit time.
 
The advantage of our method is twofold.
