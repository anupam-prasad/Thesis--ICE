...
sound wave relative to this plane. The ear closer to the sound source is referred to as the \textit{ipsilateral} ear and
is denoted by a subscript $0$. For more complex auditory systems we would require two angles $(\theta, \phi)$ to describe
the three-dimensional system but in our analysis this is unnecessary.

The ear further away from the source is referred to as the \textit{contralateral} ear and is denoted by a subscript $L$. In our subsequent analysis,
the subscripts $0$ and $L$ will correspond to the ipsi- and contralateral ears respectively, unless
otherwise specified.

\begin{figure}[ht]
 %\centering
 \includegraphics[width=.75\linewidth]{Diagrams/acousticheadmodel.png}
 \caption[Acoustic head model for geckos]{The acoustic head model for geckos. The darkly shaded region 
 is the internal cavity. The extra distance travelled by the soundwave to reach the contralateral ear is given by 
 the path ABC.}
\end{figure}
...
\subsubsection{Plane Wave Approximation}
